...
int main(int argc, char *argv[])
{
    ...
    MPI_Scatter(A, N, MPI_INT, rowA, N, MPI_INT, masterProcess, MPI_COMM_WORLD);
    // MPI_Scatter(B, 4, MPI_INT, rowB, 4, MPI_INT, masterProcess, MPI_COMM_WORLD);
    MPI_Bcast(B, N * N, MPI_INT, masterProcess, MPI_COMM_WORLD);
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        for (int j = 0; j < N; j++)
        {
            rowC[columnIndex] += rowA[j] * B[columnIndex][j];
        }

        cout << "Process " << rank << " : ";
        showRow(rowA);
        cout << "-- ";
        showRow(B[columnIndex]);
